...
def mask_and_tag(image, mask, tag=None):
    ...
    with rasterio.open(image, mode="r+") as src:
        if tag:
            ...
            print(src.tags)
        src.profile.update(nodata=0, compress="lzw")
        data = src.read() * mask
        src.write(data)
...
# SCALING functions
def rescale(array, minimum, maximum, gain=1.0):
    x = [0, 2**16 - 1]
    y = [minimum, maximum]
...
def write_new_values(image, minimum, maximum, shutter_factor=1, tag=True):
    with rasterio.open(image, mode="r+") as src:
        ...
        if tag:
            src.update_tags(
                VALUE_STRETCH_MINIMUM=minimum, VALUE_STRETCH_MAXIMUM=maximum
            )
...
def get_image_stats_multi(
    OUTDIR, sensors, n_jobs=40, nth_images=1, max_images=3000, quiet=False
):
    dfs = []
    for sensor in sensors:
        ...
        if quiet:
            stats = Parallel(n_jobs=n_jobs)(
                delayed(read_stats_extended)(image) for image in images
            )
        else:
            stats = Parallel(n_jobs=n_jobs)(
                delayed(read_stats_extended)(image)
                for image in tqdm.tqdm_notebook(images)
            )
        df_stats = pd.DataFrame(
            data=np.array(stats), columns=["mean", "min", "max", "std", "p01", "p99"]
        )
...
def get_overlapping_ds(aoi_path, projects_file, name_attribute="Dataset"):
    ...
    if "Dataset" in aoi.columns:
        aoi.drop(columns=["Dataset"], inplace=True)
    datasets = gpd.read_file(projects_file).to_crs(epsg=4326)
    overlapping_ds = gpd.sjoin(datasets, aoi, lsuffix="", how="inner").drop_duplicates(
        subset=name_attribute
    )
    return overlapping_ds


def retrieve_footprints(
    overlapping_ds,
    project_id,
    parent_data_dir,
    aoi_file,
    fp_file_regex="*full.shp",
    ...
    if len(footprint_list) == 0:
        footprint_list = list(
            (parent_data_dir / project_name).glob(
                fp_file_regex.replace(".shp", ".gpkg")
            )
        )
