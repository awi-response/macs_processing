    ...
    with zipfile.ZipFile(zip_filename, 'w') as zip_archive:
        for root, dirs, files in os.walk(directory):
            ...
            for file in files:
                ...
                if not any(excluded_dir in file_path for excluded_dir in excluded_dirs) and file not in ignored_files:
                    zip_archive.write(file_path, os.path.join(base_dir, os.path.relpath(file_path, directory)))
            for dir_path in [os.path.join(root, d) for d in dirs]:
                zip_archive.write(dir_path, os.path.join(base_dir, os.path.relpath(dir_path, directory)))
