...
def main():
    ...
    def parse_args_and_config():
        parser = argparse.ArgumentParser(description='Wrapper for Postprocessing, Moving and Archiving')
        ...
        parser.add_argument('-p', '--projects', type=str, nargs='+', required=False, help='List of project names')
        ...
        if missing_vars:
            print(f"Missing required variables: {', '.join(missing_vars)}. Please provide them via config or command line.")
            sys.exit(1)
        if not ((settings_dir and target_dir and archive_dir)):
            print('No input provided! Please provide either a config file or set settings, target and archive directory manually!')
            sys.exit(1)
